...
"@
  ...
  function Copy-SkillPayload([string]$Source, [string]$Destination) {
    # Remove only legacy payload directories that this installer previously
    # copied into the managed skill destination. They are never skill content.
    foreach ($excluded in $excludedPayloadNames) {
      $legacyPath = Join-Path $Destination $excluded
