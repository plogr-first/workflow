...
function Assert-LeaseOwner {
  if (-not $script:ControllerId) { throw 'Workflow monitor does not own a lease.' }
  try {
    $lease = Get-Content -LiteralPath $lockPath -Raw | ConvertFrom-Json
    if ([string]$lease.controller_id -ne $script:ControllerId -or ([datetime]$lease.lease_expires_at) -le (Get-Date)) {
      throw 'Workflow monitor lease ownership was lost.'
    }
  } catch { throw "Workflow monitor cannot prove lease ownership: $($_.Exception.Message)" }
}
...
function Push-MergedWorkflow($Workflow) {
  ...
  if ($policy -eq 'create_pr') {
    ...
    $pushResult = Invoke-GitCapture @('-c', 'core.quotepath=false', '-C', $project, 'push', $remote, "${featureBranch}:${featureBranch}")
    $pushOut = $pushResult.Output
    if ($pushResult.ExitCode -ne 0 -and ($pushOut -notmatch 'Everything up-to-date')) { throw "git push $remote ${featureBranch}:${featureBranch} failed: $($pushOut -join ' ')" }
  ...
  $pushResult = Invoke-GitCapture @('-c', 'core.quotepath=false', '-C', $project, 'push', $remote, $targetBranch)
  $output = $pushResult.Output
  if($pushResult.ExitCode -ne 0){throw "git push $remote $targetBranch failed: $($output -join ' ')"}
  return 'pushed'
}
function OutcomeHash([string]$Path) {
  if (-not (Test-Path -LiteralPath $Path)) { return $null }
  # Use the .NET implementation so the monitor does not depend on PowerShell
  # module auto-loading in a constrained host.
...
function Test-ManagedWorktreePath([string]$ProjectRoot, [string]$CandidatePath) {
  if ([string]::IsNullOrWhiteSpace($ProjectRoot) -or [string]::IsNullOrWhiteSpace($CandidatePath)) { return $false }
  try {
    $managedRoot = [System.IO.Path]::GetFullPath((Join-Path $ProjectRoot '.worktrees')).TrimEnd('\') + '\'
...
function Agent-Live([string]$Name) {
  try {
    $prefix = if($script:Session){ @('--session',$script:Session) } else { @() }
    $old=$ErrorActionPreference; $ErrorActionPreference='Continue'
    $raw = @(& herdr @prefix agent get $Name 2>$null)
    $code=$LASTEXITCODE; $ErrorActionPreference=$old
...
function Acquire-Lease {
  ...
  $lease = [ordered]@{ controller_id = "$PID-$([guid]::NewGuid().ToString('N'))"; pid = $PID; lease_expires_at = (Get-Date).AddMinutes(5).ToString('o') }
...
function Renew-Lease {
  if (-not (Test-Path -LiteralPath $lockPath)) { throw 'Workflow monitor lease file is missing.' }
  $lease = Get-Content -LiteralPath $lockPath -Raw | ConvertFrom-Json
  if ([string]$lease.controller_id -ne $script:ControllerId) { throw 'Workflow monitor lease ownership was lost.' }
  $lease.lease_expires_at = (Get-Date).AddMinutes(5).ToString('o')
  $tmp = "$lockPath.$([guid]::NewGuid().ToString('N')).tmp"
  $lease | ConvertTo-Json | Set-Content -LiteralPath $tmp -Encoding utf8
...
try {
  ...
  do {
    ...
    if ($workflow.mode -eq 'parallel_task' -and $workflow.next_role -eq 'matrix_tasks') {
      ...
      foreach ($sub in $matrix) {
        ...
        if ($sub.status -ne 'candidate') {
          $allCandidate = $false
          if (-not $subOutcome -and -not (Agent-Live ([string]$sub.agent_name))) { $missingMatrixAgent = $true }
        }
      }

      if ($missingMatrixAgent) { $missingMatrixTicks++ } else { $missingMatrixTicks=0 }
      if ($missingMatrixTicks -ge 3) {
        Save-State $workflow 'blocked' ''
        Append-Event 'workflow_blocked' @{ reason = 'matrix_agent_unavailable' }
        Notify 'Herdr: 并行子 Agent 不可用，工作流已阻塞等待恢复' $workflowPath 'request'
        break
      }
      ...
      if ($allCandidate) {
        ...
        try {
          if (Test-Path -LiteralPath $integrationWt) {
            Invoke-GitCapture @('-c', 'core.quotepath=false', '-C', $gitProj, 'worktree', 'remove', '--force', $integrationWt) | Out-Null
          }
          Invoke-GitCapture @('-c', 'core.quotepath=false', '-C', $gitProj, 'branch', '-D', $integrationBranch) | Out-Null
          $addResult = Invoke-GitCapture @('-c', 'core.quotepath=false', '-C', $gitProj, 'worktree', 'add', '-b', $integrationBranch, $integrationWt, 'HEAD')
          if ($addResult.ExitCode -ne 0) { throw "Failed to create integration worktree: $($addResult.Output -join ' ')" }

          # Merge each candidate branch
          $mergeFailed = $false
          foreach ($sub in $matrix) {
            $subBranch = [string]$sub.branch
            $mergeResult = Invoke-GitCapture @('-c', 'core.quotepath=false', '-C', $integrationWt, 'merge', '--no-ff', $subBranch, '-m', "Integrate subtask $($sub.id)")
            $mergeOut = $mergeResult.Output
    ...
    if ($workflow.next_role -in @('verification','publish') -and $verOutcome -and (Test-OutcomeEvidence $workflow 'verification' $verOutcome)) {
    ...
    } elseif ($workflow.next_role -in @('verification','publish') -and $verOutcome -and $workflow.last_processed.verifier_outcome_hash -ne $verHash) {
      ...
      if (($workflow.mode -eq 'research' -and $verOutcome.state -eq 'passed') -or ($workflow.mode -ne 'research' -and $verOutcome.state -eq 'merged')) {
        if($workflow.mode -ne 'research'){ try {$push=Push-MergedWorkflow $workflow; $workflow|Add-Member -Force -NotePropertyName push_status -NotePropertyValue $push} catch {$workflow.last_processed.verifier_outcome_hash=$null;$workflow|Add-Member -Force -NotePropertyName push_status -NotePropertyValue 'failed';$workflow|Add-Member -Force -NotePropertyName push_error -NotePropertyValue $_.Exception.Message;Save-State $workflow 'push_failed' 'publish';Append-Event 'push_failed' @{error=$_.Exception.Message};Notify 'Herdr: 已合并，但 git push 失败；可使用 herdr resume 重试发布' ([string]$workflow.verifier.result) 'request';break} }
        ...
        try {
          if ($workflow.task -and $workflow.task.outcome -and (Test-Path -LiteralPath ([string]$workflow.task.outcome))) {
            ...
            if ($taskOutObj.worktree_decision -eq 'isolated' -and $taskOutObj.worktree_path) {
              ...
              if ((Test-ManagedWorktreePath ([string]$workflow.project_root) $wtPath) -and (Test-Path -LiteralPath $wtPath)) {
          ...
          if ($workflow.mode -eq 'parallel_task' -and $workflow.matrix) {
            foreach ($sub in $workflow.matrix) {
              ...
              if ((Test-ManagedWorktreePath ([string]$workflow.project_root) $sWt) -and (Test-Path -LiteralPath $sWt)) {
