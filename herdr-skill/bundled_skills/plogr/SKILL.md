...
- Use this Skill when the user explicitly invokes `/plogr` or `/herdr`.
 - For dispatch, require `HERDR_ENV=1`. `plogr init` is the only setup exception.
 - Read `<project>/herdr/dispatch-profile.json` once. If absent, tell the user to run `plogr init` in that project; do not install wrappers, modify PATH, or run `npx` yourself.
...
## Observability & HUD
 ...
 - `plogr status`: Instant snapshot summary of active workflows.
 - `plogr hud` (or `status --live`): 24-bit TrueColor ANSI glowing HUD with real-time pulsating hue animation. Ideal for embedding in a 1-line Herdr bottom pane.
 - `plogr popup`: Opens full-screen floating HUD modal for Herdr popup integration (`prefix+p`).
