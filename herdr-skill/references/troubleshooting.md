...
- `HERDR_ENV is not 1`: dispatch must originate from a Herdr-managed pane. `plogr init` is an exception and can run from an ordinary project terminal.
...
- Missing profile: run `plogr init` from the project root; do not invent profile values.
