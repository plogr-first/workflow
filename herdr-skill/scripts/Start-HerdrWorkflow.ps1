...
$profilePath = Join-Path $project 'herdr\dispatch-profile.json'
 if (-not (Test-Path -LiteralPath $profilePath -PathType Leaf)) { throw "Herdr dispatch profile not found: $profilePath. Run 'plogr init' first." }
$profile = Get-Content -LiteralPath $profilePath -Raw | ConvertFrom-Json
 if ($profile.template -eq $true) { throw "Herdr dispatch profile is a template: $profilePath. Run 'plogr init' in the target project to create an initialized profile before dispatching work." }
...
$git = $profile.git
 if ($Mode -ne 'research' -and $git.repository -and -not $git.has_commit) { throw "Git is initialized but has no baseline commit in $project. Review the project, create the first commit, rerun 'plogr init', then dispatch task/bugfix." }
...
foreach($skill in $requiredSkills){
 ...
 throw "Required workflow skill '$skill' is not installed in $profilePath. Re-run 'plogr init' in this project to deploy and register skills."
  }
}
